...
begin
	...
	begin select *from NHACC where mancc= @mancc; end
	end;
...
CREATE PROCEDURE spud_VATTU_Sua
	...
	as
	begin
	--cập nhật thông tin vật tư trong bảng vật tư
	UPDATE VATTU
	SET
		tenvtu=COALESCE(@tenvtu, tenvtu),
		dvtinh=COALESCE(@dvtinh, dvtinh),
		phantram=COALESCE(@phantram, phantram)
		where mavtu=@mavtu;
	end;
--ví dụ sửa đổi tên vật tư, đơn vị tính và phần trăm của vật tư có mã "DD01"
EXEC spud_VATTU_Sua @mavtu = 'DD01',@tenvtu='Vật tư mới',@dvtinh='kg',@phantram=10.50;
--ví dụ sửa phần trăm
EXEC spud_VATTU_Sua @mavtu='DD02',@phantram=15.00;
--ví dụ sửa tên vật tư
EXEC spud_VATTU_Sua @mavtu='TL15',@tenvtu ='Vật tư đã thay đổi';
